...
@interface macOS_StoreManager()<SKRequestDelegate, SKProductsRequestDelegate>

@property (retain) SKProductsRequest *productRequest;

@end

@implementation macOS_StoreManager
...
+ (macOS_StoreManager *)sharedInstance 
{
	...
	static macOS_StoreManager * storeManagerSharedInstance;

	dispatch_once(&onceToken, ^
    {
		storeManagerSharedInstance = [[macOS_StoreManager alloc] init];
	});
...
- (void)requestDidFinish:(SKRequest *)request
{
    if ([request isKindOfClass:[SKReceiptRefreshRequest class]])
    {
        ...
        DsMapAddDouble(dsMapIndex, jId, receipt_refresh);
        
        DsMapAddDouble(dsMapIndex, jResponse, receipt_refresh_success);
